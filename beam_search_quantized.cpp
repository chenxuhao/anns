...
template<typename indexType = vidType, typename distanceType = int32_t>
std::pair<std::pair<std::vector<std::pair<indexType, distanceType>>, std::vector<std::pair<indexType, distanceType>>>, size_t>
beam_search_quantized(Graph<indexType> &G, std::vector<indexType> starting_points, QueryParams QP,
            int K, int32_t dim, size_t dsize,
            SQ8UQuantizer &quantizer_query, SQ8UQuantizer &quantizer_data,
            int po = 1, int pl = 1) {
  if (starting_points.size() == 0) {
    std::cout << "beam search expects at least one start point" << std::endl;
    abort();
  ...
  while (remain > 0 && num_visited < QP.limit) {
    ...
    for (auto a : neighbors) {
      ...
      if (po > 0 && pl > 0) {
        // Prefetch the current neighbor node's vector data
        for (int offset = 0; offset < po; offset++) {
          uint8_t *vec_ptr = (uint8_t *)quantizer_data.get_quantized_at(a) + offset * 64 / sizeof(uint8_t);;
          for (int line = 0; line < pl; line++) {
            prefetch::prefetch(vec_ptr + line * 64, prefetch::CacheLevel::L1);
          }
        }
      }
    }
...
void ANNS<T>::search(int k, int qsize, int dim, size_t npoints,
  ...
  std::cout << "Start quantizing vectors..." << std::endl;
  Timer t_quant;
  t_quant.Start();

  SQ8UQuantizer quantizer_data(Metric::IP, dim, npoints, false);
  quantizer_data.train(data_vectors);
  quantizer_data.add(data_vectors);

  t_quant.Stop();
  double quant_time = t_quant.Seconds();
  std::cout << "Quantize time: " << quant_time <<  "s" << std::endl;
  ...
  for (int query_id = 0; query_id < qsize; query_id ++) {
    ...
    if (po > 0 && pl > 0) {
      for (int i = 0; i < k; i++) {
        ...
        for (int offset = 0; offset < po; offset++) {
          uint8_t *vec_ptr = (uint8_t *)quantizer_data.get_quantized_at(i) + offset * 64 / sizeof(uint8_t);
    ...
    quantizer_query.add(query_data);

    // Pass optimized prefetch parameters
    auto res = beam_search_quantized(g, starting_points, PQ, k, dim, npoints, quantizer_query, quantizer_data, po, pl);
